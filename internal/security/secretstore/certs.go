...
func (cp *CertPair) Verify() error {
	var err error
	// Parse private key into *rsa.PrivateKey
	block, _ := pem.Decode([]byte(cp.Key))
	if block == nil {
		err = fmt.Errorf("unable to decode CertPair.Key into a PEM")
		return err
	}

	priv, err := x509.ParsePKCS8PrivateKey(block.Bytes)
	if err != nil {
		err = fmt.Errorf("unable to parse CertPair.Key: %v", err.Error())
		return err
	}

	privKey, ok := priv.(*rsa.PrivateKey)
	if !ok {
		err = fmt.Errorf("privateKey was not encoded with RSA algorithm")
		return err
	}

	// Parse certificate into *rsa.PublicKey
	block, _ = pem.Decode([]byte(cp.Cert))
	if block == nil {
		err = fmt.Errorf("unable to decode CertPair.Cert into a PEM")
		return err
	}

	certificate, err := x509.ParseCertificate(block.Bytes)
	if err != nil {
		err = fmt.Errorf("unable to parse CertPair.Cert: %v", err.Error())
		return err
	}

	// privPub is the private key's corresponding public key
	privPub := privKey.Public().(*rsa.PublicKey)
	// certPub is the public key corresponding to the certificate
	certPub := certificate.PublicKey.(*rsa.PublicKey)

	if privPub.E != certPub.E || (privPub.N.Cmp(certPub.N) != 0) {
		err = fmt.Errorf("cert pair's private key doesn't match certificate")
		return err
	}
